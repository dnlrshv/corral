...
def test_private_secret_file_and_route_allowlist(tmp_path, monkeypatch):
    ...
    path.write_text(json.dumps({"BABA_API_KEY": "fixture-private-value",
                                "UNRELATED_KEY": "must-not-forward"}))
    path.chmod(0o600)
    values = load(path)
    assert select(("BABA_API_KEY",), values) == {"BABA_API_KEY": "fixture-private-value"}
    assert "UNRELATED_KEY" not in select(("BABA_API_KEY",), values)
    monkeypatch.setenv("BABA_API_KEY", "development-fallback")
    assert select(("BABA_API_KEY",), None) == {"BABA_API_KEY": "development-fallback"}


def test_secret_file_rejects_broad_permissions_symlinks_and_bad_values(tmp_path):
    path = tmp_path / "provider-secrets.json"
    path.write_text(json.dumps({"BABA_API_KEY": "fixture"}))
    path.chmod(0o644)
